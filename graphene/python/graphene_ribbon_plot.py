...
for i in range(1,21):
	txt.append(f'NPT-Full-Ribbon/Graphene_REBO_Strain-{i}-1000.txt')

for i in range(1,21):
	txt.append(f'NVT-Full-Ribbon/Graphene_REBO_Strain.SA-NVT-{i}.txt')
...
fig, ax = plt.subplots(2,1, gridspec_kw={'height_ratios': [3, 1]},figsize=(10,7))
...
for i in range(0,runlen):
	...
	if i<20:
		if i == 0:
			a, = ax[0].plot(-Dist_Dict_xx[key], Int_Dict_yy[val]/10**9, linestyle='none',alpha = 1, marker='o', markersize=14, markerfacecolor='none', markevery=1000, c=slicedCM[0])
		ax[0].plot(Dist_Dict_xx[key], Int_Dict_yy[val]/10**9, linestyle='-',alpha = 0.4, marker='o', markersize=14, markerfacecolor='none', markevery=1000, c=slicedCM[0])
		ax[1].semilogy(Dist_Dict_xx[key], Int_Dict_Temp[val], linestyle='-',alpha = 0.4,  c=slicedCM[0])

		nptmax.append(np.max(Int_Dict_yy[val]/10**9))
	else:
		if i == 20:
			b, = ax[0].plot(-Dist_Dict_xx[key], Int_Dict_yy[val]/10**9, linestyle='none', alpha = 1, marker='s', markersize=14, markerfacecolor='none', markevery=1000, c=slicedCM[2])
		ax[0].plot(Dist_Dict_xx[key], Int_Dict_yy[val]/10**9, marker='s', markersize=14, markerfacecolor='none', markevery=1000, linestyle='-',alpha = 0.4, c=slicedCM[2])
		ax[1].semilogy(Dist_Dict_xx[key], Int_Dict_Temp[val], linestyle='-',alpha = 0.4,  c=slicedCM[2])

		nvtmax.append(np.max(Int_Dict_yy[val]/10**9))
...
ax[0].legend([a, b],[r'NPT',r'NVT'], loc='upper left', framealpha=1.0)
fig.align_ylabels(ax[:])

plt.show()


print(f'NPT max : {round(np.mean(nptmax),3)} pm {round(np.std(nptmax),3)}')

print(f'NVT max : {round(np.mean(nvtmax),3)} pm {round(np.std(nvtmax),3)}')